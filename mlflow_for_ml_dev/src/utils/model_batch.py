    ...
    for registered_model_name in registered_model_names:
        ...
        for i in range(1, n_versions):
            ...
            with mlflow.start_run(run_name=f"classifier_{i}") as run:
                ...
                mlflow.sklearn.log_model(rfc, "model", registered_model_name = registered_model_name)
            ...
            model_version_tags = {
                ...
                "task_type": random.choice(["classification", "regression", "clustering"]),
            }
            
            # set tags for the model version
            for tag_key, tag_value in model_version_tags.items():
                client.set_model_version_tag(name = registered_model_name, version = str(i), key = tag_key, value = tag_value)
